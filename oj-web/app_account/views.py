...
def user_login(request):
    ...
    raw_username = request.POST.get("username", "").strip()  # 取得username，若取得不成功，則為空字串
    password = request.POST.get("password", "")
    remember_me = request.POST.get("remember_me")  # Get the checkbox value

    # Normalize username (convert ASCII letters to lowercase) for authentication; keep original for display
    ...
    if not raw_username or not password:
        ...
        return render(
            ...
            {"context": {"error": "Missing credentials", "username": raw_username}},
        )
    ...
    # 使用小寫化的 username 做大小寫不敏感(User.username__iexact)查詢後，再以資料庫中保存的 username 進行 authenticate
    ...
    if user is None:
        logger.warning(f"登入失敗:{raw_username},{current_ip}")
        messages.error(request, "使用者名稱或密碼不正確，請重新輸入!")
        context = {"error": "Invalid credintials", "username": raw_username}
        return render(
            request,
            "app_account/registration/user_login.html",
            {"context": context},
    ...
    if forbid_login_ip and not user.is_staff:
        if not allowed_ip_ranges:
            ...
            context = {"error": "Login suspended", "username": raw_username}
            return render(
                request,
                "app_account/registration/user_login.html",
                {"context": context},
        ...
        if not ip_allowed:
            ...
            context = {"error": "IP restricted", "username": raw_username}
            return render(
                request,
                "app_account/registration/user_login.html",
                {"context": context},
...
def user_register(request):
    ...
    # POST handling
    # if request.method == "POST":
    raw_user_name = request.POST["user_name"].strip()
    password = request.POST["password"]
    full_name = request.POST["full_name"].strip()
    user_class = request.POST["user_class"].strip()
    # Normalize username to lowercase for consistency and uniqueness checks
    normalized_user_name = raw_user_name.lower()

    # Check if username exists (case-insensitive)
    if User.objects.filter(username__iexact=normalized_user_name).exists():
    ...
    try:
        ...
        logger.error(f"註冊時發生錯誤: {raw_user_name}, {str(e)}")
        messages.error(request, "註冊失敗!，請再試一次")
        return redirect("user_register")

    messages.success(request, f"{raw_user_name}註冊成功!")
