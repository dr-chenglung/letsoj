...
def contest_list(request):
    ...
    contests = Contest.objects.filter(is_visible=True)
    ...
    return render(request, "app_oj/contest_list.html", {"contests": contests,'news_list': news_list})
...
def contest_detail(request, contest_pk):
    ...
    contest_problems = ContestProblem.objects.filter(contest=contest).order_by('id_prblm_in_contest')
    # 以下3種寫法皆可??
    ...
    if (not request.user.is_staff and contest.status is ContestStatus.CONTEST_NOT_START) or not contest.is_visible:
        return redirect("contest_list")

    try:
        contest_rank = ContestRank.objects.get(submitted_by=request.user, contest=contest)
    except ContestRank.DoesNotExist:
        #還沒有submission
        contest_rank=None

    context = {"contest": contest, "contest_rank":contest_rank, "contest_problems":contest_problems}
...
def contest_problem_submit(request, contest_pk, problem_pk):
    ...
    # 可以防止入侵者通過 Http404 異常來推斷數據庫中是否存在該記錄。
    try:
        contest = Contest.objects.get(pk=contest_pk)
    except Contest.DoesNotExist:
        # raise Http404("Contest does not exist")
    ...
    sys_options = {item.option_name: item.option_value for item in MySysOptions.objects.all()}
    ...
    try:
        ...
        submission = Submission.objects.get(submitted_by=request.user, contest=contest, problem=problem)
        # 
    ...
    try:
        # 排行資訊contest rank
        contestRank = ContestRank.objects.get(submitted_by=request.user, contest=contest)
        ...
        # AC之後，不可以看自己的提交紀錄，避免考試時帳號給他人，讓他人查看其答案。
        if hide_submitted_code and contest.status != ContestStatus.CONTEST_ENDED and submit_info is not None and submit_info["is_ac"]:
            submission.source_code = ""
    except ContestRank.DoesNotExist:
        contestRank = None
    
    # 檢查 solution_release_policy 和 contest.status
    if (contest.solution_release_policy == "AFTER_CONTEST" and contest.status == ContestStatus.CONTEST_ENDED) or contest.solution_release_policy == "IMMEDIATE":
        #logger.info("可以看答案")
        pass
    else:
        #logger.info("不可以看答案")
    ...
    contest_problem.problem.description = markdown.markdown(  contest_problem.problem.description, extensions=['fenced_code', 'codehilite'] )
    #logger.info( contest_problem.problem.description )
    
    contest_problem.problem.input_output_description = markdown.markdown(  contest_problem.problem.input_output_description, extensions=['fenced_code', 'codehilite'] )
    

    context = {"contest": contest, "submission": submission, "contest_problem": contest_problem}
...
def judge(sourceCode, stdin, expected_output, language_id):
    ...
    try:
        ...
        response_post = requests.request("POST", judger_url+'?base64_encoded=true', data=judge0_payload)
...
def submit_to_judger(request):
    ...
    elif contest.status == ContestStatus.CONTEST_NOT_START and not request.user.is_staff:
    ...
    try:
        # 看看有沒有提交過
        # Accepted之後，仍可以提交多次，但是只有最後一次的結果會被記錄，會影響排名
        ...
        #     return JsonResponse(judge_response_info)
        # 前端也可以管制:<button type="button" id="submitMyCode" class="btn btn-primary" {% if contest.status == "已結束" or submission.judge_status_description == "Accepted" %}disabled{% endif %}>提交程式碼...</button>
   ...
   # 送給Judger執行
    ...
    for test_case in test_cases:  
        ...
        try:
            ...
            task_token = judge(source_code, test_case['stdin'], test_case['expected_output'], problem.language.judge_id)
...
def get_submission_result(request):
    #logger.info("取得提交結果.........")
    submission_tokens = eval(request.POST.get("task_token")) # 字串格式轉換為list可用json.loads()
    ...
    for task_token in submission_tokens:
        ...
        if  status == JudgeStatus.IN_QUEUE or status == JudgeStatus.PROCESSING:
            ...
            # 答案錯誤
            judge_response_info["judge_compile_output"] = "未通過~~有部分或全部測資答案錯誤，請再仔細檢查程式碼!"
            break
        elif judge_result['status']['id'] == JudgeStatus.COMPILE_ERROR_1:
            # 編譯等錯誤 編號6
            if judge_result['compile_output'] is not None:
                judge_response_info["judge_compile_output"] =  base64ToString(judge_result['compile_output']) # 編譯錯誤，會有程式碼錯誤行，學生可檢查該行程式碼是否有語法錯誤!
            else:
                # 編譯錯誤，但是沒有編譯訊息(應該沒有這樣的情況才對)，避免這樣的情況發生轉換錯誤噴錯
                judge_response_info["judge_compile_output"] = "編譯錯誤，請檢查程式碼是否有語法錯誤!"
            break
        elif judge_result['status']['id'] == JudgeStatus.INTERNAL_ERROR:
            # 伺服器內部錯誤 編號
            judge_response_info["judge_compile_output"] = "伺服器內部錯誤，請通報管理員，謝謝!"  # 
            break
        else:
            # 其他情況:執行期間錯誤Runtime Error, 此時judge_result['compile_output']是None
            judge_response_info["judge_compile_output"] = "執行期間發生了錯誤，請檢查程式碼的邏輯是否有錯誤~"
            break
            
    # JudgerSUCCESS成功完成判題(不管有沒有通過測資，Judger就是成功完成任務了!)，準備好資料回傳   
    judge_response_info["result_status"] = "JudgerSUCCESS"
    judge_response_info["judge_status_description"] = judge_result['status']['description']
    ...
    # 紀錄排行資訊
    ### Update contest rank
    contestRank, sucess = ContestRank.objects.update_or_create(submitted_by=user, contest=contest)
    ...
    if submit_info:
        ...
        if judge_response_info['judge_status'] == JudgeStatus.ACCEPTED:
            ...
            submit_info["ac_time"] = (submission.submitted_at - contest.start_time).total_seconds()
            contestRank.total_time = submit_info["ac_time"]
        elif judge_response_info['judge_status'] != JudgeStatus.IN_QUEUE and judge_response_info['judge_status'] != JudgeStatus.PROCESSING and judge_response_info['judge_status'] != JudgeStatus.INTERNAL_ERROR:
    ...
    # 第一次提交，新建submit_info
        ...
        contestRank.submission_count += 1
        submit_info = {"is_ac": False, "ac_time": 0, "error_count": 0, "qz_prblm_id": contest_problem.id_prblm_in_contest}
...
def get_contest_ranking(request, contest_id):
    ...
    contest_ranks = ContestRank.objects.filter(contest=contest).select_related("submitted_by").exclude(submitted_by__in=staff).order_by("-accepted_count", "total_time")

    for userRecord in contest_ranks:
        if userRecord.accepted_count != 0: #有提交成功才有時間紀錄 若無時間紀錄在網頁會顯示"尚未提交"
...
def user_contests_summary(request):
    ...
    user = request.user
    attended_contest_ranks  = ContestRank.objects.filter(submitted_by=user).order_by('-contest__display_seq')

    # 比對哪個該參加的公開競賽沒有參加
    all_public_contests = Contest.objects.filter(is_visible=True)
    ...
    contests_not_attended = all_public_contests.exclude(id__in=attended_contest_ranks.values_list('contest__id', flat=True))
    ...
    for rank in attended_contest_ranks:
        ...
        # 查詢比賽資料
        all_users_contest_ranks = ContestRank.objects.filter(contest=contest).select_related("submitted_by").exclude(submitted_by__in=staff).order_by("-accepted_count", "total_time")
        ...
        user_rank = None
        average_time = 0
        if rank.accepted_count != 0: #有提交成功才有時間紀錄 若無時間紀錄在網頁會顯示"尚未提交"
            # 計算平均時間
