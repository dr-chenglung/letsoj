...
def contest_problem_submit(request, contest_pk, problem_pk):
    ...
    # hide_submitted_code字符串內容是 "False" 或 "True"，使用字典映射來安全地轉換這些字符串為布爾值，不可用eval
    try:
...
judge0_payload = {
    ...
    "language_id": language_id, # Java 可能是 62 或 89 (視 Judge0 版本)
    "additional_files": encoded_files, # 裡面包含 sqlite-jdbc.jar
    "compiler_options": "-cp .:sqlite-jdbc.jar", # 告訴 javac 引用這個 jar
    "run_options": "-cp .:sqlite-jdbc.jar",      # 告訴 java 執行時引用這個 jar
}

# 讀取並將包含驅動程式(.jar)或資料庫(.sqlite)的壓縮檔進行 Base64 編碼
with open("my_test_env.zip", "rb") as zip_file:
    encoded_files = base64.b64encode(zip_file.read()).decode('utf-8')

judge0_payload = {
    "source_code": sourceCode,
    "language_id": language_id,
    "additional_files": encoded_files, # <--- 加上這個欄位
    # ... (其他設定)
}
"""
...
def judge(sourceCode, stdin, expected_output, language_id, additional_files=None):
    ...
    # 若有帶入額外附檔 (例如 jdbc jar 與 sqlite)，則加入 payload
    if additional_files:
        judge0_payload["additional_files"] = additional_files
        # 不使用 compiler_options 與 command_line_arguments，因為直接將 zip 解壓縮至當前目錄 `.` 即可讓 Java 自動載入 .class 檔案

    
    # 將 test_cases 轉換為 JSON
    # judge0_payload = json.dumps(judge0_payload)
    # logger.info(judge0_payload)
    logger.info(f"Keys in payload before requests.request: {list(judge0_payload.keys())}")

    # 送給Judger執行 若發生例外，會被捕捉到
    try:
        response_post = requests.request(
            "POST", judger_url + "?base64_encoded=true", json=judge0_payload
        )
        logger.debug(f"Judge0 response status: {response_post.status_code}")
        logger.debug(f"Judge0 response body: {response_post.text}")
        if response_post.status_code >= 400:
            logger.error(
                "Judge0 request failed with status %s: %s",
...
def submit_to_judger(request):
    ...
    # 確認此題目確實屬於此競賽，避免使用者竄改 problem_id/contest_id 提交不相干的題目
    if not ContestProblem.objects.filter(contest=contest, problem=problem).exists():
        judge_response_info["result_status"] = "InvalidProblem"
        judge_response_info["judge_status_description"] = "此題目不屬於該競賽！"
    ...
    try:
        ...
        # Plain Text (judge_id=43) 題型：只能提交一次，已交過就擋掉
        # 管理者除外，方便測試
        if getattr(problem.language, "judge_id", None) == 43 and not request.user.is_staff:
            judge_response_info["result_status"] = "AlreadySubmitted"
            judge_response_info["judge_status_description"] = "此題僅能提交一次，無法再次提交！"
            return JsonResponse(judge_response_info)
    ...
    # Plain Text (judge_id=43) 選擇題：不套用程式樣板，並去除前後空白
    # 否則 template 的 prepend/append 會在答案前後插入換行，導致與 expected_output 比對失敗
    # 可以自己寫比對程式碼，不必依賴 Judge0的比對。
    if getattr(problem.language, "judge_id", None) == 43:
        source_code = (source_code or "").strip() # 如果 source_code 是 None，則使用空字符串，並去除前後空白
    elif problem.template.strip():
    ...
    # === 資料庫題目:新增以下這段邏輯 ===
    encoded_additional_files = None
    # 透過 ORM 判斷關聯的分類中，是否有名稱包含「資料庫」的分類
    is_database_problem = problem.categories.filter(name__icontains="資料庫").exists()
    logger.info(f"是否為資料庫題型: {is_database_problem}")
    
    # 取得目前 app_oj 的目錄路徑
    APP_DIR = os.path.dirname(os.path.abspath(__file__))

    # 組合出正確且跨平台的絕對路徑
    env_zip_path = os.path.join(APP_DIR, "judger_env", "sqlite_env.zip")
        
    if is_database_problem:
        try:

            with open(env_zip_path, "rb") as zip_file:
                encoded_additional_files = base64.b64encode(zip_file.read()).decode('utf-8')
        except Exception as e:
            logger.error(f"讀取資料庫題型 ZIP 附檔失敗: {e}")
    # ==========================
    ...
    logger.info("提交測資.........")
    test_case = problem.get_test_cases()[0]  # 目前只支援單一測資
    try:
        task_token = judge(
            source_code,
            test_case["stdin"],
            ...
            encoded_additional_files # <--- 動態帶入
        )
        judge_response_info["task_token"] = str(task_token)
    except Exception as error:
...
def get_submission_result(request):
    logger.info("取得提交結果.........")
    task_token = json.loads(request.POST.get("task_token"))
    logger.info(f"token: {task_token}")
    ...
    try:
        ...
        if judge_result.get("stderr"):
            try:
                stderr_decoded = base64.b64decode(judge_result["stderr"]).decode(
                    "utf-8"
                )
            except Exception:
                stderr_decoded = judge_result["stderr"]
            logger.error(f"Judge0 stderr: {stderr_decoded}")
        if judge_result.get("compile_output"):
            try:
                compile_decoded = base64.b64decode(
                    judge_result["compile_output"]
                ).decode("utf-8")
            except Exception:
                compile_decoded = judge_result["compile_output"]
            logger.error(f"Judge0 compile_output: {compile_decoded}")
    except Exception as error:
    ...
    elif status == JudgeStatus.ACCEPTED:
        # 答案正確
        judge_response_info["judge_compile_output"] = "通過~~測資正確!"
    elif status == JudgeStatus.WRONG_ANSWER:
        # 答案錯誤
        judge_response_info["judge_compile_output"] = (
            "未通過~~測資答案錯誤，請再仔細檢查程式碼!"
        )
    elif status == JudgeStatus.COMPILE_ERROR_1:
        # 編譯等錯誤 編號6
    ...
    elif status == JudgeStatus.INTERNAL_ERROR:
        # 伺服器內部錯誤 編號
        judge_response_info["judge_compile_output"] = (
            "伺服器內部錯誤，請通報管理員，謝謝!"  #
        )
    ...
    # 紀錄排行資訊
    ### Update contest rank
    # 用 transaction.atomic + select_for_update 對 ContestRank 該列加鎖，
    # 避免同一使用者快速重複提交時，「讀取→修改→寫回」互相覆蓋導致計數錯亂(競態)
    with transaction.atomic():
        # 先確保該列存在，再用 select_for_update 鎖定，後續請求須等本交易結束才能讀取
        ContestRank.objects.get_or_create(submitted_by=user, contest=contest)
        contestRank = ContestRank.objects.select_for_update().get(
