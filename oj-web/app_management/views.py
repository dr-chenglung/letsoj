...
def problem_submit(request, pk):
    ...
    problem.input_output_description = markdown.markdown(
        problem.input_output_description, extensions=["fenced_code", "codehilite", "tables"]
    )
...
def export_problems_to_excel(request):
    ...
    if request.method == "POST":
        ...
        fields = [
            ...
            if not isinstance(field, (ForeignObjectRel)) and field.name not in ["contests"]
        ]
        logger.info(fields)
        ...
        try:
            ...
            for row in ws.iter_rows(min_row=2, values_only=False):
                    ...
                    if field_name == "categories":
                        ...
                        if cell.value is not None and cell.value != "":
                            try:
                                ...
                                dt = datetime.strptime(str(cell.value), "%Y/%m/%d %H:%M:%S")
                                # Convert to timezone-aware datetime
                                dt = timezone.make_aware(dt)
                                data[field_name] = dt
                            except (ValueError, TypeError) as e:
                                logger.warning(f"Failed to parse datetime {cell.value}: {e}")
                ...
                if problem_id:
                    ...
                    logger.info(f"Created new problem with auto-generated ID {problem.id}")
                # problem.save()
                logger.info(problem)
                # Process categories
                if category_names:
...
def import_contest_submissions(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # 檢查 contest_id 是否有效
            if not contest_id:
                messages.error(request, 'JSON 檔案中未包含 contest_id，請確認檔案格式正確')
                return render(request, 'app_management/import_contest_submissions.html')
    ...
    if request.method == 'POST':
        ...
        if action == 'download':
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                ...
                contest_json = json.dumps(contest_metadata, ensure_ascii=False, indent=2)
                zip_file.writestr(
                    f'contest_metadata_{contest_id}.json',
                    contest_json.encode('utf-8')
                )

                # 添加 problems JSON
                problems_json = json.dumps(problems_data, ensure_ascii=False, indent=2)
                zip_file.writestr(
                    f'contest_problems_{contest_id}.json',
                    problems_json.encode('utf-8')
                )

                # 添加 contest_problems JSON
                contest_problems_json = json.dumps(contest_problems_data, ensure_ascii=False, indent=2)
                zip_file.writestr(
            ...
            # 準備下載回應
            response = HttpResponse(zip_buffer.getvalue(), content_type='application/zip')
            response['Content-Disposition'] = f'attachment; filename="contest_export_{contest_id}.zip"'
            return response
        ...
        messages.success(
            request,
            mark_safe(f'✅ <strong>競賽匯出預覽完成</strong><br>'
                     f'📋 競賽：{contest.title} (ID: {contest_id})<br>'
...
def import_contest(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            contest_problems_data = json.loads(contest_problems_file.read().decode('utf-8'))
            ...
            for problem_data in problems_data:
                try:
                    ...
                    problem = Problem.objects.create(
                        ...
                        input_output_description=problem_data['input_output_description'],
                    ...
                    if problem_data.get('categories'):
                        ...
                        for cat_name in problem_data['categories']:
                            if cat_name not in category_map:
                                category, created = ProblemCategory.objects.get_or_create(name=cat_name)
            ...
            # 調試信息
            logger.info(f"匯入完成，準備顯示結果: contest={contest.title}, imported_problems={imported_problems}")
