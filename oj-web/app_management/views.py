...
from app_oj.views import stringToBase64, base64ToString
    ...
    if request.method == "POST":
        ...
        try:
            ...
            for row in ws.iter_rows(min_row=2, values_only=False):
                # logger.info(row)
                # reset data per row to avoid leaking values from previous rows
                data = {}
                ...
                if not raw_username:
                    logger.warning("跳過匯入：缺少 username 的列")
                    continue
                normalized_username = raw_username.lower()
                data["username"] = normalized_username

                try:
                    # case-insensitive lookup to find existing user regardless of stored case
                    user = User.objects.filter(username__iexact=normalized_username).first()
                    if user:
                        # ensure username is stored in lowercase
                        user.username = normalized_username
                        user.password = make_password(data.get("password") or "")
                        user.full_name = data.get("full_name")
                        user.user_class = data.get("user_class")
                        user.save()
                    else:
...
def submit_to_judger(request):
    ...
    # 取得目前 app_oj 的目錄路徑 (因為檔案放在 app_oj)
    import os
    env_zip_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "app_oj", "judger_env", "sqlite_env.zip")
...
def export_problems_to_excel(request):
    ...
    if request.method == "POST":
        ...
        # Write header row
        # fields = ['id','category', 'title', 'description','std_input','std_output','sample_code','created_at','modified_at','is_sample_code_visible','is_public','languages','template','note']

        # ['contests', 'contestproblem', 'submission', 'id', 'category', 'title', 'description', 'std_input', 'std_output', 'sample_code', 'created_at', 'modified_at', 'is_sample_code_visible', 'is_public', 'languages', 'template', 'note', 'categories']
...
def export_submissions(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request, f"競賽編號 {contest_id} 不存在")
            return render(request, "app_management/export_submissions.html")
    ...
    context = {"export_result": export_result}
    return render(request, "app_management/export_submissions.html", context)
...
def import_submissions(request):
    ...
    if request.method == "POST":
        ...
        if not submissions_file or not ranks_file:
            ...
            return render(request, "app_management/import_contest_and_problems.html")
        ...
        return render(request, "app_management/import_submissions.html")

    return render(request, "app_management/import_submissions.html")


@staff_member_required
def export_contest_and_problems(request):
    """
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request, f"競賽編號 {contest_id} 不存在")
            return render(request, "app_management/export_contest_and_problems.html")
    ...
    context = {"export_result": export_result}
    return render(request, "app_management/export_contest_and_problems.html", context)


@staff_member_required
def import_contest_and_problems(request):
    """
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # 取代/新建 Contest(保留原 id,不刪競賽本身 → 不會連坐刪提交與排名)
            ...
            # 只清題目關聯(安全:ContestProblem 刪除不會 cascade 到 Submission),稍後依匯入重建
            ...
            return render(request, "app_management/import_contest_and_problems.html", context)
